...
=
\frac{1}{2}m_1 v_{1,f}^2 + \frac{1}{2}m_2 v_{2,f}^2
$$
...
$$F(t) = F_0 \sin\left(\frac{\pi t}{\Delta t}\right) \quad \text{para } 0 \le t \le \Delta t$$
...
$$J = \int_{0}^{\Delta t} F_0 \sin\left(\frac{\pi t}{\Delta t}\right) dt = F_0 \left[ -\frac{\Delta t}{\pi} \cos\left(\frac{\pi t}{\Delta t}\right) \right]_{0}^{\Delta t}$$

$$J = -\frac{F_0 \Delta t}{\pi} (\cos(\pi) - \cos(0)) = -\frac{F_0 \Delta t}{\pi} (-1 - 1) = \frac{2 F_0 \Delta t}{\pi}$$

Sustituyendo nuestros valores:

$$J = \frac{2 (50\pi \text{ N})(0.1 \text{ s})}{\pi} = 10.0 \text{ N}\cdot\text{s}$$

1. Determinación de la Fuerza Media: La fuerza constante equivalente que provocaría el mismo cambio de momentum es:
   $$F_{\text{media}} = \frac{J}{\Delta t} = \frac{10.0 \text{ N}\cdot\text{s}}{0.1 \text{ s}} = 100.0 \text{ N}$$
2. Efecto en el Momentum Individual: Por el teorema del impulso y el cambio de momentum ($\vec{J} = \Delta \vec{p}$):

    Para la Masa 2: Recibe un impulso positivo de $+10.0 \text{ N}\cdot\text{s}$.
    
    $$\Delta p_2 = J \implies m_2 v_{2f} - 0 = 10.0 \implies 3.0 v_{2f} = 10.0 \implies v_{2f} = 3.33 \text{ m/s}$$
    
    Para la Masa 1: Por tercera ley de Newton (fuerzas internas del sistema), recibe un impulso de $-10.0 \text{ N}\cdot\text{s}$.
    
    $$\Delta p_1 = -J \implies m_1 v_{1f} - m_1 v_{1i} = -10.0 \implies 2.0 v_{1f} - 2.0(6.0) = -10.0$$
    
    $$2.0 v_{1f} = 12.0 - 10.0 = 2.0 \implies v_{1f} = 1.0 \text{ m/s}$$

3. Verificación de la Conservación del Momentum del Sistema: 


$$P_{\text{inicial}} = m_1 v_{1i} + m_2 v_{2i} = (2.0)(6.0) + 0 = 12.0 \text{ kg}\cdot\text{m/s}$$

$$P_{\text{final}} = m_1 v_{1f} + m_2 v_{2f} = (2.0)(1.0) + (3.0)(3.33) = 2.0 + 10.0 = 12.0 \text{ kg}\cdot\text{m/s}$$

**¡El momentum total del sistema se conservó perfectamente!**

1. Diagnóstico del Choque y Coeficiente de Restitución: Calculamos el coeficiente de restitución ($e$):

$$e = - \frac{v_{2f} - v_{1f}}{v_{2i} - v_{1i}} = - \frac{3.33 - 1.0}{0 - 6.0} = \frac{2.33}{6.0} \approx 0.389$$
...
1. **El fin de la fase de compresión:** Desde $t = 0$ hasta este punto, la masa trasera ($m_1$) se movía más rápido que la delantera ($m_2$), por lo que se venían acercando y aplastando el parachoques. En el cruce, al igualarse las velocidades, el parachoques ya no se puede comprimir más.

2. **Coincidencia con la velocidad del Centro de Masa ($v_{cm}$):** En ese milisegundo exacto, como ambos cuerpos se mueven solidarios a la misma velocidad, esa velocidad es idéntica a la velocidad del centro de masa del sistema ($v_{cm} = 2.4 \text{ m/s}$).

3. **Inicio de la fase de restitución:** Inmediatamente después de este punto, el parachoques libera la energía elástica acumulada, "empujando" a $m_2$ para que se aleje a mayor velocidad y frenando aún más a $m_1$ hasta que se separan por completo en $t = 0.1 \text{ s}$.
...
Demuestre que se trata de una colisión perfectamente inelástica calculando su coeficiente de restitución.
...
$$(60.0 \text{ kg})(4.0 \text{ m/s}) + (40.0 \text{ kg})(0) = (60.0 \text{ kg} + 40.0 \text{ kg}) v_f$$

$$240.0 \text{ kg}\cdot\text{m/s} = (100.0 \text{ kg}) v_f$$

Despejamos $v_f$:

$$v_f = \frac{240.0}{100.0} = 2.4 \text{ m/s}$$
...
$$e = - \frac{2.4 - 2.4}{0 - 4.0} = \frac{0}{-4.0} = 0$$
